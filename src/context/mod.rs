...
impl<'a> ContextBuilder<'a> {
    ...
    pub fn build_context(&self, task: &str, options: &ContextOptions) -> Result<TaskContext> {
        ...
        let related_nodes = if !entry_points.is_empty() {
            self.graph.find_related(&entry_points, options.max_nodes / 2)?
        } else {
            Vec::new()
        };
    ...
    fn extract_keywords(&self, task: &str) -> Vec<String> {
        ...
        let stop_words = [
            ...
            "find", "get", "look", "see", "use", "make", "want", "fix", "add",
            "update", "change", "modify", "implement", "create", "delete", "remove",
        ];
    ...
    fn build_code_blocks(&self, nodes: &[Node], options: &ContextOptions) -> Result<Vec<CodeBlock>> {
